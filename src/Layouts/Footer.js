import React from "react";
// import { Container, Row, Col } from 'react-bootstrap'; // Xóa dòng này
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome";
import {
  faFacebookF,
  faYoutube,
  faTiktok,
  faGithub,
} from "@fortawesome/free-brands-svg-icons";
import { APP_CONFIG } from "../Constants";

const AppName = APP_CONFIG.APP_NAME;

const Footer = () => {
  return (
    <footer className="bg-dark text-white pt-5 pb-3">
      {/* Thay Container bằng div với class container */}
      <div className="container">
        {/* Thay Row bằng div với class row */}
        <div className="row mb-4">
          {/* Logo Section */}
          {/* Thay Col bằng div với class col-md-3 */}
          <div className="col-md-3 text-start">
            <div className="d-flex align-items-center mb-3">
              <img
                src={APP_CONFIG.APP_LOGO} // Replace with your F8 logo path
                alt="F8 Logo"
                width="36"
                height="36"
                className="me-2"
              />
              <span className="fw-bold fs-5">{APP_CONFIG.APP_SLOGAN}</span>
            </div>
            <p className="small mb-1 text-white-50">
              Điện thoại: {APP_CONFIG.PHONE_NUMBER}
            </p>
            <p className="small mb-1 text-white-50">
              Email: {APP_CONFIG.EMAIL}
            </p>
            <p className="small text-white-50">Địa chỉ: {APP_CONFIG.DIA_CHI}</p>
            <img
              src="data:image/png;base64," // Replace with your DMCA badge path
              alt="DMCA Protected"
              width="100"
            />
          </div>

          {/* About F8 Section */}
          <div className="col-md-2">
            <h5 className="mb-3">VỀ {APP_CONFIG.APP_NAME}</h5>
            <ul className="list-unstyled small">
              {APP_CONFIG.ABOUT.map((item, index) => (
                <li key={index} style={{ marginBottom: "5px" }}>
                  <a
                    href={item.link}
                    className="text-white text-decoration-none text-white-50"
                  >
                    {item.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Products Section */}
          <div className="col-md-2">
            <h5 className="mb-3">SẢN PHẨM</h5>
            <ul className="list-unstyled small">
              {APP_CONFIG.PRODUCT.map((item, index) => (
                <li key={index} style={{ marginBottom: "5px" }}>
                  <a
                    href={item.link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-white text-decoration-none text-white-50"
                  >
                    {item.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Tools Section */}

          <div className="col-md-2">
            <h5 className="mb-3">CÔNG CỤ</h5>
            <ul className="list-unstyled small">
              {APP_CONFIG.CONG_CU.map((tool, index) => (
                <li key={index} style={{ marginBottom: "5px" }}>
                  <a
                    href={tool.link}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-white text-decoration-none text-white-50"
                  >
                    {tool.label}
                  </a>
                </li>
              ))}
            </ul>
          </div>

          {/* Company Info Section */}
          <div className="col-md-3">
            <h5 className="mb-3">
              CÔNG TY CỔ PHẦN CÔNG NGHỆ GIÁO DỤC {AppName}
            </h5>
            <p className="small mb-1 text-white-50">Mã số thuế: 0109922901</p>
            <p className="small mb-1 text-white-50">
              Ngày thành lập: 06/14/2025
            </p>
            <p className="small mb-3 text-white-50">
              Lĩnh vực hoạt động: Giáo dục, công nghệ - lập trình.
            </p>
            <p className="small text-white-50">
              Chúng tôi tập trung xây dựng và phát triển các sản phẩm mang lại
              giá trị cho cộng đồng lập trình viên Việt Nam.
            </p>
          </div>
        </div>

        {/* Bottom Bar */}
        <div className="row align-items-center pt-3">
          <div className="col-md-6">
            <p className="text-white-50 small mb-0">
              © 2018 - 2025 {AppName}. Nền tảng học lập trình hàng đầu Việt Nam
            </p>
          </div>
          <div className="col-md-6 text-end">
            <a href="#" className="text-white me-4">
              <FontAwesomeIcon
                icon={faYoutube}
                style={{ color: "#FF0000" }}
                size="lg"
              />
            </a>
            <a href="#" className="text-white me-4">
              <FontAwesomeIcon
                icon={faFacebookF}
                style={{ color: "#1877F2" }}
                size="lg"
              />
            </a>
            <a href="#" className="text-white me-4">
              <FontAwesomeIcon
                icon={faTiktok}
                style={{ color: "#E6E6E6" }}
                size="lg"
              />
            </a>
            <a
              href="https://github.com"
              target="_blank"
              rel="noopener noreferrer"
              className="text-white"
            >
              <FontAwesomeIcon
                icon={faGithub}
                style={{ color: "#ffffff" }}
                size="lg"
              />
            </a>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
